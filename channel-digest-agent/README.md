...
A busy group channel is unreadable. A hundred messages a day across half a dozen channels, most of
them social, a handful carrying a deadline, a payment, a link, or a required action. Reading
everything costs an hour a day. Skipping it costs a missed deadline.

The deployed instance runs against high-volume operational channels written in Hebrew, for a reader
who does not read Hebrew. That is a deliberately hard case: steady traffic, a foreign language,
right-to-left script, mixed media, and real consequences when something slips through.
...
**1. Channel names are resolved live, with a frozen fallback.** `Fetch Group Name` asks the API for
...
**2. The two media branches rejoin the text branch, they do not replace it.** `After Images` and
...
**3. `Commit Processed IDs` sits after `Send Digest`, and the media forward sits after the
commit.** That order is not cosmetic — see below.
...
**Summarize the context, reproduce the payload verbatim.** Prose gets compressed. Actionable data
gets copied character for character: full URLs, exact amounts, phone numbers, access codes,
...
⚠️ **Measured, not asserted — and the result was not the one anyone predicted.** This is a
prompt-level rule, so it was put on a bench: **[40 labelled cases](./evals)**, exact substring
matching, no model judging another model.

The deployed prompt reproduced **73.7 %** of payloads. Here is the twist. The failure was not
paraphrasing at all. The model discarded whole messages as irrelevant, including an assigned ticket
number, a cost centre and an escalation address. A retention rule lifted that to **92.1 %**, and no
further prompt iteration is expected to reach 100 %.
...
is indistinguishable from a broken pipeline, and it sends the reader back to the channel — breaking
the zero round-trip rule.

**Bilingual rendering is an engineering problem.** Hebrew runs right to left. A line starting with
Hebrew flips the entire block in the messaging client and the digest becomes unreadable. Every
output line therefore opens with a Latin-script label, which forces left-to-right layout. This one
is invisible until you ship to a real reader in a real client.
...
out, the images and documents it mentions are forwarded to the same destination. The description is
what lets the reader decide; the file is what lets them act.
...
`$getWorkflowStaticData()` even when an execution **fails** — the `workflowExecuteAfter` hook saves
it with no check on status. A node that marks messages processed upfront therefore destroys them
the moment anything downstream errors: the provider rate-limits, and those messages are gone from
every future cycle, silently. Read the memory upstream, carry the candidate IDs through the
